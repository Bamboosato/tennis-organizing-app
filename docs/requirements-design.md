...
- Guest はメンバー管理、登録メンバーからのメンバー選択を利用できない。対戦表作成後のPDF出力は利用可能とする。
...
- Guestの場合:
  - メンバー情報を使わない。
  - 人数分の番号表示名を生成する。1桁番号は2桁にゼロ埋めし、性別を識別できるよう末尾に `F` / `M` を付ける。例: `01F`, `02F`, `05M`
...
- PDFはログインユーザー、Guestのどちらでも利用可能とする。
- PDF向きは A4縦とする。
- ログインユーザーの表示名はニックネームとする。
- Guestの表示名は `01F` / `05M` のような番号 + 性別記号とする。
- ログインユーザー向けPDFはブルー系の独自レイアウト、Guest向けPDFは `tennis-matchup-app` と同様の `R | コート1 | ... | 休憩` の表形式とする。
- ログインユーザー向けPDFでは、休憩者が多い場合に備え、休憩表示は独立カードにする。
...
| 対戦表結果 | 可 | 可 | 組合せ結果表示 |
| PDF出力 | 可 | 可 | ログインユーザーはニックネーム表示PDF、Guestはtennis-matchup-app相当の番号表示PDF |
...
| N-011 | PDF | GuestでPDF出力するとtennis-matchup-app相当の表形式で出力できる | Guest向け出力フォーマットを満たすこと |
...
| E-003 | Guest | Guestでメンバー登録・登録メンバー選択を実行しようとする | Guest制限が破れないこと |
